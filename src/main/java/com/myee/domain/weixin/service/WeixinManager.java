...
import com.myee.domain.restaurant.dao.TableTypeDao;
import com.myee.domain.restaurant.model.TableType;
import com.myee.domain.weixin.WeixinService;
import com.myee.domain.weixin.dao.RShopDao;
import com.myee.domain.weixin.dao.RWaitTokenDao;
import com.myee.domain.weixin.dao.WFeedBackDao;
import com.myee.domain.weixin.model.*;
...
public class WeixinManager extends RedisOperation implements WeixinService {
    ...
    public Collection<ShopDetail> allStoreOfClient(Long clientId, Long orgId) {
        ...
        if (null == storeMap || storeMap.isEmpty()) {
            List<RShop> storeList = storeDao.findByClientId(clientId);
            storeMap = Maps.newHashMap();
            for (RShop store : storeList) {
                List<TableType> typeList = tableTypeDao.findTypeByOrgId(store.getOrgID());
                ShopDetail shopDetail = store.toDto();
                shopDetail.setWaitList(toWaitList(typeList));
                storeMap.put(store.getId(), shopDetail);
            }
    ...
    private List<TableWait> toWaitList(List<TableType> typeList) {
        List<TableWait> waitList = Lists.transform(typeList, new Function<TableType, TableWait>() {
            ...
            public TableWait apply(TableType tableType) {
                ...
                wait.setTableTypeId(tableType.getTableTypeID());
                return wait;
            }
        });
        return waitList;
    ...
    public ShopDetail storeOfClient(long clientId, long storeId) {
        ...
        if (null == shopDetail) {
            RShop store = storeDao.findOne(storeId);
            if (null != store) {
                shopDetail = store.toDto();
                List<TableType> typeList = tableTypeDao.findTypeByOrgId(store.getOrgID());
                shopDetail.setWaitList(toWaitList(typeList));
    ...
    public RWaitToken enqueue(long shopId, int dinerCount, String openId) {
        ...
        try {
            ...
            waitTokenDao.saveAndFlush(rwToken);
            ...
            hsetSimple(rwToken.getWaitTokenId().toString(), redisKey, map.get("eTime"));
            hset(redisKey.trim(), rwToken.getWaitTokenId().toString(), waitToken, map.get("eTime"));
            //去查询进展
