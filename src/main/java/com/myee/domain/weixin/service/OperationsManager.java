package com.myee.domain.weixin.service;
...
import com.myee.domain.weixin.dao.RWaitTokenDao;
import com.myee.domain.weixin.model.RWaitToken;
import com.myee.util.TimeUtil;
...
public class OperationsManager extends RedisOperation implements OperationsService {
    ...
    public ResponseData take(WaitToken waitToken) {
        ...
        RWaitToken rWaitToken = waitTokenDao.saveAndFlush(WeixinManager.convertTo(waitToken,date.getTime()/1000));
        //然后二维码的数字码放Redis，跟identityCode唯一码绑定
//        append(waitToken.getSceneId(), waitToken.getIdentityCode());
        //然后identityCode放Redis，跟排号的key绑定
        Map<String,Date> map1 = new HashMap<String,Date>();
        map1 = TimeUtil.getAfterDate(date);
        append(waitToken.getIdentityCode(), redisKey, map1.get("eTime"));
        hset(redisKey, waitToken.getToken(), waitToken, map1.get("eTime"));
        WxMpQrCodeTicket a = null;
        String imgUrl = "";
        try {
            a = wxMpService.qrCodeCreateTmpTicket(Integer.parseInt(waitToken.getSceneId()),2592000);
            imgUrl = "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=" + a.getTicket();
        } catch (WxErrorException e) {
            e.printStackTrace();
        }
        
        if (rWaitToken.getWaitTokenId() != null) {
            return ResponseData.successData(imgUrl);
        } else {
...
//                if (waitToken.getShopId().equals(sortedTokens.get(0).getShopId()) && waitToken.getToken().equals(sortedTokens.get(0).getToken())) {
            WaitToken redisWaitToken = map.get(waitToken.getToken());
            if(redisWaitToken != null){
                    hdelete(redisKey, waitToken.getToken());
                    //改状态2
                    waitTokenDao.updateState(WaitTokenState.SKIP.getValue(),waitToken.getClientId(),waitToken.getShopId(),waitToken.getToken(),redisWaitToken.getTimeTook(), new Date().getTime()/1000);
                }
            ...
            if (waitToken.getToken().equals(sortedTokensW.get(0).getToken()) && waitToken.getShopId().equals(sortedTokensW.get(0).getShopId())) {
                //改状态2
                waitTokenDao.updateState(WaitTokenState.SKIP.getValue(),waitToken.getClientId(),waitToken.getShopId(),waitToken.getToken(),sortedTokensW.get(0).getTimeTook(),new Date().getTime()/1000);
            }
            for(WaitToken tokenInfo : sortedTokensW) {
                Map<String,Date> map1 = new HashMap<String,Date>();
                map1 = TimeUtil.getAfterDate(new Date(tokenInfo.getTimeTook()));
        ...
        if(afterMap != null) {
            ...
            for(RWaitToken rw : rwList) {
                if(rw.getOpenId() != null && rw.getOpenId().trim().length() > 0) {
                    ...
                    oneMap.put("openId",rw.getOpenId());
                    openIdList.add(msgMap);
                }
            }
        }
    ...
    public ResponseData repast(WaitToken waitToken, long tableId) {
        ...
        if(map != null ) { //如果redis里有数据，直接从Redis里拿
            ...
            if (waitToken.getShopId().equals(sortedTokens.get(0).getShopId()) && waitToken.getToken().equals(sortedTokens.get(0).getToken())) {
                hdelete(redisKey, waitToken.getToken());
                //改状态2
                backStatus = waitTokenDao.updateState(WaitTokenState.REPASTING.getValue(),waitToken.getClientId(),waitToken.getShopId(),waitToken.getToken(),sortedTokens.get(0).getTimeTook(),new Date().getTime()/1000);
            }
        } else { //如果redis里没有数据，从MySql里同步
            List<RWaitToken> tokenList = waitTokenDao.selectAllTokenByInfo(waitToken.getClientId(), waitToken.getShopId(), waitToken.getTableTypeId(), WaitTokenState.WAITING.getValue());
            List<RWaitToken> sortedTokens = orderingByTook2.sortedCopy(tokenList);
            ...
            if (waitToken.getToken().equals(sortedTokensW.get(0).getToken()) && waitToken.getShopId().equals(sortedTokensW.get(0).getShopId())) {
                //改状态2
                backStatus = waitTokenDao.updateState(WaitTokenState.REPASTING.getValue(),waitToken.getClientId(),waitToken.getShopId(),waitToken.getToken(),sortedTokensW.get(0).getTimeTook(),new Date().getTime()/1000);
            }
            for(WaitToken tokenInfo : sortedTokensW) {
