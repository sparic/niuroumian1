...
import com.myee.domain.weixin.WeixinService;
import com.myee.domain.weixin.model.RWaitToken;
import com.myee.domain.weixin.service.OperationsManager;
import com.myee.dto.ClientAjaxResult;
import com.myee.util.ConverterUtil;
...
//            logger.info("inMessage.toString()->" + inMessage.toString());
//            /*if(inMessage.getEventKey().equals("11111")) {
//                wxMpService.customMessageSend(WxMpCustomMessage.TEXT().toUser(inMessage.getFromUserName()).content("测试消息！").build());
//            }*/
////            jumpUrl(null,resp);
//            // 消息类型
//            String msg = null;
//            if(inMessage.getMsgType() != null && inMessage.getMsgType().trim().length() != 0 && inMessage.getEvent()!= null && inMessage.getEvent().equals(WxConsts.BUTTON_CLICK)) {
//                msg = checkLatestDevelopments(inMessage.getFromUserName(), WaitTokenState.WAITING.getValue());
//                wxMpService.customMessageSend(WxMpCustomMessage.TEXT().toUser(inMessage.getFromUserName()).content(msg).build());
//            }
//            //正则表达式判断是否包含字母数字特殊符号
////            String pattern = "^(?![a-zA-Z0-9]+$)(?![^a-zA-Z/D]+$)(?![^0-9/D]+$).{6}$";
//            //正则表达式判断是否包含字母数字
//            String pattern = "^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6}$";
//            if(inMessage.getMsgType() != null && inMessage.getMsgType().equals("text") && inMessage.getContent() != null && inMessage.getContent().length()==6 && inMessage.getContent().matches(pattern)) {
//                msg = checkLatestDevelopments(inMessage.getContent());
//                wxMpService.customMessageSend(WxMpCustomMessage.TEXT().toUser(inMessage.getFromUserName()).content(msg).build());
//            }
//
//            if(inMessage.getEvent()!= null && inMessage.getEvent().equals(WxConsts.BUTTON_SCANCODE_WAITMSG)) {
//                if(inMessage.getEventKey().equals("V1001_QUERY_CODE")) {
//                    String identityCode = inMessage.getScanCodeInfo().getScanResult();
//                    //将该二维码绑定扫码的微信OpenId
//                    if(wxService.bondQrCodeByScan(identityCode, inMessage.getFromUserName())) {
//                        msg = checkLatestDevelopments(identityCode);
//                    } else {
//                        msg = "该唯一码已过期或无效，查询进度失败!";
//                    }
//                    wxMpService.customMessageSend(WxMpCustomMessage.TEXT().toUser(inMessage.getFromUserName()).content(msg).build());
//                }
//            }
        ...
        try {
            ...
            if ("raw".equals(encryptType)) {
                ...
                if(inMessage.getMsgType() != null && inMessage.getMsgType().equals(WxConsts.XML_MSG_EVENT) && inMessage.getEvent() != null && inMessage.getEvent().equals(WxConsts.EVT_SCAN)) {
                    String identityCode = ConverterUtil.toAnyConversion(new BigInteger(inMessage.getEventKey()),new BigInteger("36"));
                    //将该二维码绑定扫码的微信OpenId
                ...
                if(inMessage.getMsgType() != null && inMessage.getMsgType().equals(WxConsts.XML_MSG_EVENT) && inMessage.getEvent() != null && inMessage.getEvent().equals(WxConsts.EVT_SUBSCRIBE) && inMessage.getTicket() != null) {
                    String identityCode = ConverterUtil.toAnyConversion(new BigInteger(inMessage.getEventKey().substring(8,inMessage.getEventKey().length())),new BigInteger("36"));
                    //将该二维码绑定扫码的微信OpenId
                    Map<String,Object> msgMap = new HashMap<String,Object>();
                    if(wxService.bondQrCodeByScan(identityCode, inMessage.getFromUserName())) {
                        msgMap = checkLatestDevelopments(identityCode);
